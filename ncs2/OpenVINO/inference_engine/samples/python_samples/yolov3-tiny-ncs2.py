...
def build_argparser():
    ...
    parser.add_argument("-m", "--model", help="Path to an .xml file with a trained model.", type=str, default='/data/github_repos/yolov3-tiny-fit-ncs/ncs2/OpenVINO/inference_engine/samples/python_samples/' \
                'yolov3-tiny-ncs-mini-pack/FP16/20190218182235_TinyYoloV3NCS.xml')
    parser.add_argument("-i", "--input", help="Path to a image/video file. (Specify 'cam' to work with camera)", type=str, default='test.jpg')
...
class YoloV3Params:
    ...
    def __init__(self, param, side):
        ...
        self.anchors = [10, 25, 20, 50, 30, 75, 50, 125, 80, 200, 150, 150] if 'anchors' not in param else [float(a) for
                                                                                                            a in param[
                                                                                                                'anchors'].split(
                ',')]
        self.side = side
...
def main():
    ...
    assert len(net.inputs.keys()) == 1, "Sample supports only Zenith YOLO V3 Tiny based single input topologies"
    assert len(net.outputs) == 2, "Sample supports only Zenith YOLO V3 Tiny based double output topologies"
    ...
    while cap.isOpened():
        ...
        # print('time used in ncs per image : ' + str(det_time))
        ...
        if exec_net.requests[cur_request_id].wait(-1) == 0:
            ...
            for layer_name, out_blob in output.items():
                ...
                if (layer_name == 'layer16-conv'):
                    ...
                    internalresults2 = detector.Detect(out2.astype(np.float32), 33, blockwd * 2, blockwd * 2, classes,
                                                       imgw, imgh, threshold, nms, blockwd * 2)
                    pyresults1 = [BBox(x, 1, 1, 0, 0) for x in internalresults2]
            
            pyresults_all = pyresults1 + pyresults1
            # print(pyresults_all)
            pre_dic = {}
            list_all = []
            for i in np.arange(6):
                list_temp = []
                for bbx in pyresults_all:
                    if (bbx.objType == i):
                        list_temp.append([bbx.left, bbx.top, bbx.right, bbx.bottom, bbx.confidence])
                if (len(list_temp) == 0):
                    continue
            ...
            fpsImg = cv2.putText(imdraw, "%.2ffps" % fps_moving_average, (70, 50), cv2.FONT_HERSHEY_SIMPLEX, 1,
                                 (0, 0, 0), 1)
