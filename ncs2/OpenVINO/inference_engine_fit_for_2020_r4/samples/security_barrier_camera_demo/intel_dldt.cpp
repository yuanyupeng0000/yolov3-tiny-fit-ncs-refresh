...
#include "./test_lib_app/intel_dldt.h"

using namespace InferenceEngine;
//FLAGS_m_str = "/data/github_repos/ncs2/intel_models/vehicle-license-plate-detection-barrier-0106/FP32/vehicle-license-plate-detection-barrier-0106.xml";
//FLAGS_m_va_str = "/data/github_repos/ncs2/intel_models/vehicle-attributes-recognition-barrier-0039/FP32/vehicle-attributes-recognition-barrier-0039.xml";
//FLAGS_m_lpr_str = "/data/github_repos/ncs2/intel_models/license-plate-recognition-barrier-0001/FP32/license-plate-recognition-barrier-0001.xml";
...
int main_process(int argc, char *argv[], std::string &strInput, std::vector<LPObject> &vLPObjects){
    try {
        ...
        if (FLAGS_i == "cam") {
            ...
            /*
            std::vector<std::string> videoFiles;
            cv::Mat frame = cv::imread(strInput, cv::IMREAD_COLOR);
            images.push_back(frame);
            */
        ...
        std::vector<std::string> pluginNames = {
                "CPU", ////FLAGS_d,
                "CPU", ////VehicleAttribs.enabled() ? FLAGS_d_va : "",
                "CPU",////LPR.enabled() ? FLAGS_d_lpr : ""
        };
        ...
        do {
            ...
            for (auto && lp : licensePlates) {
                ...
                LPObject temp;
                temp.x = lp.location.x;
                temp.y = lp.location.y;
                temp.with = lp.location.width;
                temp.height = lp.location.height;
                temp.text = lp.text;
                vLPObjects.push_back(temp);
                }
            }
...
int main(int argc, char *argv[]) {
    ...
    try {
        ...
        do {
            ...
            if (!FLAGS_no_show) {
                //cv::Mat dst_;
                //cv::resize(displayImage.getMat(), dst_, cv::Size(1920*4,1080*4));
                cv::imshow("Detection results", displayImage.getMat());
                cv::imwrite("result.jpg", displayImage.getMat());
