...
static int decode_write(AVCodecContext *avctx, AVPacket *packet)
{
    ...
    while (1) {
		...
		AVPixelFormat em_type = AVPixelFormat(tmp_frame->format);
			
        size = av_image_get_buffer_size(em_type, tmp_frame->width, tmp_frame->height, 1);
...
bool get_frame_ffmpeg(AVFormatContext *input_ctx , AVCodecContext *decoder_ctx)
{
	...
	/* actual decoding and dump the raw data */
    if ((iret = av_read_frame(input_ctx, &packet)) < 0)
        bret = false;

    if (iret == packet.stream_index)
        iret = decode_write(decoder_ctx, &packet);
...
bool open_camera(char *path, VideoCapture &cap)
{
    ...
    if (!cap.isOpened()) {
        //prt(info, "camera open faild.\n");
        cap.release();
    }
    else{
        ret = true;
        //prt(info, "camera open is ok.\n");
    }
