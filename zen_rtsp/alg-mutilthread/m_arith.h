...
typedef struct tagCfgs
{
	...
	float near_point_length;//近距点距离
	float actual_distance[8][MAX_IMAGE_HEIGHT];
	Uint16 uActualDetectLength[8];//实际流量线圈长度
	Uint16 uActualTailLength[8];//实际尾部占有线圈长度
	float actual_degree_length[MAX_IMAGE_HEIGHT];//实际刻度线对应的距离
	...
	bool bMaskLaneImage;//是否设置车道掩模图像
	bool bMaskDetectImage;//是否设置检测区域掩模图像
...
extern  bool  flag;
////////////////////////////////////////////////////////////
extern Uint16 ArithProc(Uint16 ChNum, unsigned char *pInFrameBuf, unsigned char *pInuBuf, unsigned char *pInvBuf, int FrameWidth, int FrameHeight, RESULTMSG* outBuf, Int32 outSize, ALGCFGS *pCfgs, ALGPARAMS *pParams);
...
static void SpeedCaculate(Uint16 LaneID, ALGCFGS *pCfgs, ALGPARAMS	*pParams, CPoint m_ptend[], int imgW, int imgH, int flag);
static Uint16 GetImagePointValue(Int16 iCol,Int16 iRow,Uint16 Lines,Uint16 Columns,Uint8 *ImagePtr);
static void iSubStractImage(Uint8 *puSourceImage,Uint8 *puTargetImage, Uint8 *puMaskImage, Uint32 nThreshold, Int16 laneID, Int16 width, Int16 height);
#define POINTSIZE 16
typedef struct args{
	ALGCFGS *pCfgs;
	ALGPARAMS *pParams;
...
static bool Color_deviate(Uint8 *uImage,Uint8 *vImage,int width,int height);////txl,20150714

int alg_mem_malloc(m_args *);
int alg_mem_free(m_args *arg_arg);
Uint16 ArithDetect(ALGCFGS* pCfgs, Mat img, int* result);
#endif
