...
class ObjectWrapper():
    ...
    def __init__(self, model_xml, model_bin):
        ...
        self.N = ObjectWrapper.N
        self.time0 = ObjectWrapper.time0
    ...
    def get_infer_indexes(self, idx):
        if(ObjectWrapper.infer_idx_steps[idx] % 2 == 0):
            return idx*2 + 1
        return idx*2
    ...
    def Detect(self, frame, idx):
        ...
        nmsed_between_layer_results = []
        ObjectWrapper.infer_idx_steps[idx] += 1
        #------------------------------------------------------------------------------
        if(ObjectWrapper.infer_idx_steps[idx] < 3): 
            print("[ INFO ] current infer id {0}".format(self.get_infer_indexes(idx)))     
            self.exec_net.start_async(self.get_infer_indexes(idx), inputs={self.input_blob: in_frame})
            return []
        else:
            print("[ INFO ] wait id {0}".format(self.get_infer_indexes(idx)))
            status = self.infer_requests[self.get_infer_indexes(idx)].wait()
            print("[ INFO ] wait status {0}".format(status))
            print("[ INFO ] fetch result id {0}".format(self.get_infer_indexes(idx)))
            output = self.exec_net.requests[self.get_infer_indexes(idx)].outputs
            print("[ INFO ] fetch result success")
            print("[ INFO ] current infer id {0}".format(self.get_infer_indexes(idx)))
            self.exec_net.start_async(self.get_infer_indexes(idx), inputs={self.input_blob: in_frame})
            for layer_name, out_blob in output.items():
                blockwd = 13
        ...
        print("[ INFO ] benchmark point 666")
        self.N = self.N + 1
        duration = time.time() - self.time0
        print("N:{0}, FPS:{1}".format(self.N, self.N/duration))
