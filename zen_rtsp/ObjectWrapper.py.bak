...
class ObjectWrapper():
    ...
    def Detect(self, frame, idx):
        ...
        # Start inference
        print('previous_request_id:{0};cur_request_id:{1};detector:{2}'.format(self.previous_request_id, self.cur_request_id, idx))
        #------------------------------------------------------------------------------
        print("detector {0} start_async start".format(idx))
        self.exec_net.start_async(self.cur_request_id, inputs={self.input_blob: in_frame})
        print("detector {0} start_async end".format(idx))
        nmsed_between_layer_results = []
