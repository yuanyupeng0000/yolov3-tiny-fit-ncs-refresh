...
void *client_send_udp(void *node_data,void *data)
{
    ...
    SendDataToClient(p_data->fd, (char*) &read_data_pack, sizeof(real_data_pack_t) );
	//prt(info, "cam[%d] 1--SendDataToClient end", index);
   //////////////////////////////////////////////////////////////////////////////////////
	...
	for(int i = 0; i < DETECTLANENUMMAX; i++) {
	    ...
	    if (1 == five_flag) {
	        ...
	        if (ex_static_info.real_test_five[index].lane[i].Vehnum < 2) { //0或1
	            if (0 == head_tm) {
	                head_tm  = 300; //5分钟
	            }
	        }
...
void handle_change(unsigned char * p_obj,int class_type,int index)
{
        ...
        switch(class_type){
            ...
            case CLASS_mCameraDelete:
                {
                    client_cmd_flag = CHANGE_CAM_STATUS;
                }
                break;
        default :
                break;
...
int handle_buffer(int s_index, unsigned char *buf, int len)
{
    //prt(info, "handle buffer");
        ...
        if (index >= CAM_MAX || cmd_p->objlen < 0) {
             prt(info, "get cmd type %x, length %d,index %d data len: %d", type,len,cmd_p->objnumber, cmd_p->objlen);
             return -1;
        }
        ...
        if(type==STARTREALDATA) { //||type==HEART||type==SHUTDOWN) {
